...
-->
<template>
  <div class="predicted-data-parameters my-1">
    <div>
      My predicted data includes:
      <br />
      <div class="ml-1 mt-1">
        <label>
          <input value="weather only" type="radio" v-model="data_available" />
          weather only.
        </label>
        <br />
        <label>
...
@Component
export default class PredictedDataParams extends Vue {
  @Prop() type: string;
  ...
  get parameters() {
    const params: Record<string, string | Record<string, string>> = {
      ...
      parameters: {
        data_available: this.data_available,
